...
function love.load(...)
    local ok, err = pcall(_origLoad, ...)
    if not ok then _bootError = "load: " .. tostring(err) end
end

function love.update(...)
    if _bootError then return end
    local ok, err = pcall(_origUpdate, ...)
    if not ok then _bootError = "update: " .. tostring(err) end
end

function love.draw()
    if _bootError then
        love.graphics.setColor(1, 0, 0)
        love.graphics.print(_bootError, 10, 10)
        return
    end
    local ok, err = pcall(_origDraw)
    if not ok then
        _bootError = "draw: " .. tostring(err)
        love.graphics.setColor(1, 0, 0)
        love.graphics.print(_bootError, 10, 10)
    end
end
